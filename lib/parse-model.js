...
function parseModel(model) {
  if (model.type && typeof model.type === "string") {
    ...
    return modelAttributeTypes[type](value, options, model)
  }
  const modelKeys = Object.keys(model)
  const parsedModel = modelKeys.reduce((accumulator, currentValue) => {
    const { type, value, options } = model[currentValue]
    const _value = modelAttributeTypes[type](value, options, accumulator)
...
/**
 * arrayItemFrom - Allows for selection of an item from an Array based on the value of a previously
 * defined and parsed item.  NOTE: the item being used as the index MUST come earlier in the model json
 * -> parseModel can be either a Node or a Leaf.
 *
 * @param  {Object} model   A model Node
 * @param  {Object} options [size: Number]
 * @param  {Object} set The currently assembled object
 * @return {Array} A parsed model
 */

function arrayItemFrom(model, options, set) {
  ...
  if(options.offset) {
    // - 1 to account for the fact arrays start with zero
    val = val - 1;
  }
