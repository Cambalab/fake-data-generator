...
function append(model, options) {
  const output = parseModel(model)
  if (isLeafModel(options.text)) {
    return `${output}${parseModel(options.text)}`
  } else {
    return `${output}${options.text}`
  }
}
...
function prepend(model, options) {
  const output = parseModel(model)
  if (isLeafModel(options.text)) {
    return `${parseModel(options.text)}${output}`
  } else {
    return `${options.text}${output}`
  }
}
